...
export default function Overview() {
  ...
  const items: Item[] = [
    { num: 1, title: "AI Founders", body: "Investimos em Founders, preferencialmente brasileiros, que estão criando startups AI-First." },
    { num: 2, title: "Equipes Técnicas", body: "Equipes complementares, técnicas e com profundidade em IA. A equipe deve ser composta por 2–4 Co-Founders, com ao menos 1 Tech Founder (CTO)." },
    { num: 3, title: "MVP Validado", body: "Startups com indícios de tração (early revenue, POCs em andamento, contratos assinados, primeiras vendas, etc.) e MVP validado." },
    { num: 4, title: "Mercado Grande", body: "Agnósticos de setor, desde que o mercado seja grande e tenha o Brasil como mercado target." },
  ];
    ...
    >
      <style jsx global>{`
        @media (min-width: 1181px) and (max-width: 1439px) {
          #overview .wrap { margin-left: 26px; }
          #overview { --gutter-left: calc(26px + 32px); }
        }
      ...
      <div className="mx-auto max-w-[560px] px-5 min-[1181px]:hidden">
        ...
        <p className="text-white/85 text-[14px] leading-relaxed max-w-[56ch] mx-auto mt-3 text-center">
          Nossa <span className="italic font-semibold" style={{ color: HILIGHT }}>missão</span> é guiar os ousados na trilha da
        ...
        <div className="mt-10">
          {items.map((it, idx) => (
            <div key={it.num} className="relative flex flex-col items-center text-center py-8">
              <div className="w-10 h-10 rounded-full flex items-center justify-center border text-white select-none" style={{ borderColor: HILIGHT }}>
                <span className="font-semibold text-[14px]">{it.num}</span>
              </div>
              <h4 className="font-serif text-[16px] leading-tight mt-3">{it.title}</h4>
              <p className="text-white/80 text-[14px] leading-relaxed mt-2 max-w-[60ch] px-3">{it.body}</p>
              {idx < items.length - 1 && <div aria-hidden className="w-px mt-8" style={{ height: 84, backgroundColor: HILIGHT }} />}
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="hidden min-[1181px]:block">
        ...
        >
          <div className="grid grid-cols-4" style={{ gap: "var(--col-gap)" }}>
            {items.map((it, idx) => (
              <div key={it.num} className="relative text-center px-2">
                {idx < items.length - 1 && (
                  <span
                    aria-hidden
                    className="absolute"
                    style={{
                      ...
                      width: "calc(100% + var(--col-gap) - var(--dot) - var(--gapL) - var(--gapR))",
                      height: "var(--stroke)",
                ...
                <h4 className="font-serif leading-tight mt-3" style={{ fontSize: "var(--h4)" }}>
                  {it.title}
                </h4>
                <p className="text-white/80 leading-relaxed mt-2" style={{ fontSize: "var(--p)" }}>
                  {it.body}
                </p>
              </div>
            ))}
