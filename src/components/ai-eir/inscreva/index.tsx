  ...
  return (
    <section id="inscreva" className="relative text-white" style={{ backgroundColor: BG }}>
      <style jsx global>{`
        ...
        #inscreva .card{
          position:relative;border-radius:16px;background:rgba(0,0,0,.08);
          box-shadow:0 8px 20px rgba(0,0,0,.35);
        }
        #inscreva .card::before,#inscreva .card::after{
          content:"";position:absolute;left:0;right:0;height:1px;
          background:linear-gradient(90deg,rgba(255,98,77,0) 0%,rgba(255,98,77,0.35) 20%,rgba(255,98,77,0.85) 50%,rgba(255,98,77,0.35) 80%,rgba(255,98,77,0) 100%);
        }
        #inscreva .card::before{top:0;} #inscreva .card::after{bottom:0;}
      `}</style>

      <div aria-hidden className="session-line" />

      {/* MOBILE (agora com o botão que antes era do DESKTOP) */}
      <div className="px-5 py-10 text-center min-[820px]:hidden">
        <div className="mx-auto max-w-[680px]">
          <h1 className="font-serif text-[30px] leading-tight">
            Inscreva-se agora no <span className="italic font-semibold" style={{ color: HILIGHT }}>AI EiR</span>
          </h1>
          <p className="mt-3 text-[16px] text-white/90">
            Venha construir sua startup de IA com a gente!
          </p>

          {/* estilo do botão que ANTES era do desktop */}
          <a
            href="#apply-eir"
            className="mt-6 inline-flex items-center justify-center gap-2 rounded-md font-semibold h-12 px-6 min-w-[220px] text-[15px] border border-gray-400 text-white bg-transparent transition-all duration-200 hover:-translate-y-0.5 hover:scale-[1.01] hover:brightness-110 hover:shadow-md hover:shadow-black/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF624D]/40"
          >
      ...
      {/* DESKTOP (agora com o botão cheio que ANTES era do mobile) */}
      <div className="hidden min-[820px]:block py-12">
        <div className="mx-auto px-8 max-w-[920px]">
          <div className="card px-10 py-10 text-center">
            ...
            <p className="mt-3 text-[18px] text-white/90">Venha construir sua startup de IA com a gente!</p>

            {/* estilo do botão que ANTES era do mobile */}
            <a
              href="#apply-eir"
              className="mt-6 inline-flex items-center justify-center gap-2 rounded-md font-semibold h-12 px-5 min-w-[200px] text-[15px] border-2 border-[#FF624D] bg-black text-white transition-all duration-200 hover:-translate-y-0.5 hover:scale-[1.01] hover:brightness-110 hover:shadow-md hover:shadow-black/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF624D]/60"
            >
